...
### [Общие вопросы по сборкам](Interviews/InterviewAnswers/DotNet/ОбщиеВопросы.md)
1. Что такое сборка?
...
10. Что такое замыкание? Как оно работает?
...
12. В чём будет проблема с кодом
```csharp
lock(message){
 await Task.Sleep(5)
}**
...
1. Что это? Когда используется?
   ...
   - На каких потоках происходит асинхронное выполнение метода в разных частях кода?
