...
**GAC (Global Assembly Cache)** — это глобальный кэш сборок в .NET Framework, который представляет собой централизованное хранилище для сборок со строгими именами (Strong-Named Assemblies), предназначенных для совместного использования несколькими приложениями на одном компьютере.

### Назначение и концепция GAC

GAC решает несколько ключевых задач:

| Проблема                      | Решение через GAC                               |
|-------------------------------|-------------------------------------------------|
| "DLL Hell" - конфликты версий | Разные версии одной сборки могут сосуществовать |
| Дублирование сборок           | Единая копия для всех приложений                |
| Управление версиями           | Централизованный контроль версий сборок         |
| Безопасность                  | Повышенный уровень доверия к сборкам в GAC      |

### Расположение и структура GAC

```text
# Физическое расположение в Windows
C:\Windows\Microsoft.NET\assembly\

# Структура каталогов
GAC_32/          # Для 32-битных сборок
GAC_64/          # Для 64-битных сборок  
GAC_MSIL/        # Для нейтральных к архитектуре сборок
NativeImages_*/  # Для нативных образов (NGEN)
```

### Требования для размещения в GAC

#### 1. Строгое имя (Strong Name)

```csharp
// Сборка должна быть подписана строгим именем
...
```csharp
// Манифест сборки должен содержать версию
...
|-----------------------------|-----------------------------------------------------|
| Совместное использование    | Одна копия сборки для многих приложений             |
| Управление версиями         | Side-by-side execution разных версий                |
| Повышенное доверие          | Сборки в GAC считаются более надежными              |
| Централизованное обновление | Обновление в одном месте затрагивает все приложения |

#### Недостатки и проблемы

| Недостаток                 | Описание                                     |
|----------------------------|----------------------------------------------|
| Сложность развертывания    | Требуются права администратора               |
| "DLL Hell" возвращается    | При неправильном управлении версиями         |
| Усложнение диагностики     | Сложнее отследить, какая версия используется |
| Ограниченная переносимость | Только для .NET Framework на Windows         |
...
```mermaid
graph LR
A[.NET Framework<br/>GAC] --> B[.NET Core<br/>Локальное развертывание]
B --> C[.NET 5+<br/>Пакеты NuGet]
```
...
* GAC — это особенность .NET Framework, в .NET Core/+ он отсутствует
* Требует строгого именования сборок (Strong Name)
* Решает проблему Side-by-Side execution разных версий
* Упрощает развертывание общих библиотек в enterprise-среде
* Имеет как преимущества, так и серьезные недостатки

#### Для Senior-разработчика важно понимать:

* Когда использование GAC оправдано
* Альтернативы в современных версиях .NET
* Проблемы, которые могут возникнуть при использовании GAC

GAC представляет собой классический компромисс между централизацией и гибкостью в управлении зависимостями.
